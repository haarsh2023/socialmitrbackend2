...
const { registerWalletRoutes, creditWallet, pendingCredit, confirmPendingCredit, initiateRazorpayPayout } = require('./wallet');
...
app.post('/api/razorpay/release-payment', auth, async (req, res) => {
  ...
  try {
    ...
    const { data: contract } = await supabase.from('contracts')
      .select('*, jobs(title), creators(name,user_id,upi_id,bank_account,bank_ifsc), businesses(business_name,user_id)')
      .eq('id', contract_id).single();
