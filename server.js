...
if (!process.env.JWT_SECRET) {
  console.warn('⚠️ WARNING: JWT_SECRET is not defined. Using insecure fallback. Set JWT_SECRET in Render env vars.');
}
...
console.log('   FRONTEND_URL        :', process.env.FRONTEND_URL ? '✅ ' + process.env.FRONTEND_URL : '⚠️  Not set (all origins allowed)');
console.log('');
...
if (process.env.FRONTEND_URL) {
  // Support comma-separated multiple URLs: FRONTEND_URL=https://a.com,https://b.com
  process.env.FRONTEND_URL.split(',').map(u => u.trim()).filter(Boolean).forEach(u => allowedOrigins.push(u));
}
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // In development (no FRONTEND_URL set) allow everything
...
// ─── HEALTH CHECK ─────────────────────────────────────────────────────────────
app.get('/health', (req, res) => res.json({ status: 'ok', version: '5.0.0', uptime: Math.floor(process.uptime()) }));
