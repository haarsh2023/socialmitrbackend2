...
const { registerWalletRoutes, creditWallet, pendingCredit, settlePending, confirmPendingCredit, confirmManualPayout, initiateRazorpayPayout } = require('./wallet');
...
console.log('   GROQ_API_KEY        :', process.env.GROQ_API_KEY && process.env.GROQ_API_KEY !== 'gsk_your_key_here' ? '✅ set (AI features enabled)' : '❌ MISSING (AI features disabled)');
...
console.log('   SITE_URL            :', process.env.SITE_URL ? '✅ ' + process.env.SITE_URL : '⚠️  NOT SET (emails will use local request host)');
console.log('   SMTP CONFIG         :', `${process.env.SMTP_HOST || 'smtp-relay.brevo.com'}:${process.env.SMTP_PORT || '587'} (secure: ${process.env.SMTP_SECURE || 'false'})`);
console.log('');
...
const smtpSecure = process.env.SMTP_SECURE === 'true'; // Defaults to false for port 587 (uses STARTTLS)
...
// Non-blocking verify — startup never waits on SMTP
setTimeout(() => {
  transporter.verify(err => err
    ? console.error(`❌ SMTP Connection Failed (${smtpHost}:${smtpPort}):`, err.message, '\n👉 Try setting SMTP_PORT=587 or 2525 in Render env vars.')
    : console.log(`   SMTP                : connected to ${smtpHost}:${smtpPort}`)
  );
}, 3000);
...
// Return a proper Promise from sendEmail
function sendEmail(to, tpl) {
  if (!to || !tpl) return Promise.resolve();
  const from = `"${process.env.MAIL_FROM_NAME || 'SocialMitr'}" <${process.env.MAIL_FROM_ADDRESS || process.env.BREVO_LOGIN}>`;
  return transporter.sendMail({ from, to, subject: tpl.subject, html: tpl.html, text: tpl.text })
    .catch(e => console.error('Email error (non-fatal):', e.message));
}


// ─── GROQ AI HELPER (OpenAI-compatible) ───────────────────────────────────────
function callGroq(prompt, maxTokens = 400) {
  return new Promise((resolve, reject) => {
    const apiKey = process.env.GROQ_API_KEY;
    if (!apiKey || apiKey === 'gsk_your_key_here' || apiKey.includes('xxxx')) {
      return reject(new Error('Add your GROQ_API_KEY to .env — get it free at console.groq.com'));
    }
...
app.post('/api/register/editor', authLimiter, async (req, res) => {
  ...
  try {
    ...
    const dashboardLink = `${getSiteUrl(req)}/creator-dashboard.html?autotoken=${token}`;
...
app.post('/api/register/business', authLimiter, async (req, res) => {
  ...
  try {
    ...
    const dashboardLink = `${getSiteUrl(req)}/business-dashboard.html?autotoken=${token}`;
...
// ─── ADMIN PAYMENTS — full payment list with creator/business/job details ────
app.get('/api/admin/payments', async (req, res) => {
  const secret = req.headers['x-admin-secret'];
  if (!process.env.ADMIN_SECRET || secret !== process.env.ADMIN_SECRET)
    return res.status(403).json({ success: false, message: 'Forbidden' });
  try {
    // Get all payments joined with contracts, jobs, creators, businesses
    const { data: payments, error } = await supabase
      ...
      .select(`
        ...
        contracts (
          ...
          agreed_amount,
          jobs ( title ),
          creators ( name, upi_id, bank_account, bank_ifsc, razorpay_account_id ),
    ...
    const flat = (payments || []).map(p => ({
      ...
      amount:               p.amount || p.contracts?.agreed_amount,
      status:               p.status,
      ...
      note:                 p.note,
      contract_status:      p.contracts?.status,
      job_title:            p.contracts?.jobs?.title,
      creator_name:         p.contracts?.creators?.name,
      upi_id:               p.contracts?.creators?.upi_id,
      bank_account:         p.contracts?.creators?.bank_account,
      bank_ifsc:            p.contracts?.creators?.bank_ifsc,
      razorpay_account_id:  p.contracts?.creators?.razorpay_account_id,
      business_name:        p.contracts?.businesses?.business_name,
    }));

    res.json({ success: true, payments: flat });
  } catch (err) {
...
// ─── ADMIN MARK-PAID — manually mark a payout as done ───────────────────────
app.post('/api/admin/mark-paid', async (req, res) => {
  const secret = req.headers['x-admin-secret'];
  if (!process.env.ADMIN_SECRET || secret !== process.env.ADMIN_SECRET)
    return res.status(403).json({ success: false, message: 'Forbidden' });
  const { payment_id, contract_id, note } = req.body;
  if (!payment_id) return res.status(400).json({ success: false, message: 'payment_id required' });
  try {
    // 1. Fetch payment and contract details to get creator_id and amount
    const { data: payment, error: pErr } = await supabase
      .from('payments')
      .select('*, contracts(creator_id, jobs(title), businesses(business_name))')
      .eq('id', payment_id)
      .single();

    if (pErr || !payment) return res.status(404).json({ success: false, message: 'Payment not found' });

    const creatorId = payment.contracts?.creator_id;
    const amount = payment.amount || payment.contracts?.agreed_amount;
    const jobTitle = payment.contracts?.jobs?.title || 'Job';

    if (!creatorId) return res.status(400).json({ success: false, message: 'Creator ID not found for this payment' });

    // 2. Credit Creator Wallet (This is the ACTUAL money move in our system)
    console.log(`[ADMIN] Manual Payout: Crediting ₹${amount} to creator ${creatorId} for ${jobTitle}`);
    try {
      await creditWallet(creatorId, Number(amount), contract_id || payment.contract_id, `Manual payout by admin for "${jobTitle}"`);
    } catch (wErr) {
      console.error('Wallet credit error in admin mark-paid:', wErr.message);
      return res.status(500).json({ success: false, message: 'Wallet credit failed: ' + wErr.message });
    }

    // 3. Update Payment Status to 'paid_to_creator'
    await supabase.from('payments').update({
      status:      'paid_to_creator',
      released_at: new Date().toISOString(),
      note:        note || 'Manual payout by admin (wallet credited)'
    }).eq('id', payment_id);

    // 4. Update Contract Status if needed
    if (contract_id || payment.contract_id) {
      await supabase.from('contracts').update({
        status:       'completed',
        completed_at: new Date().toISOString()
      }).eq('id', contract_id || payment.contract_id);
    }

    // 5. Notify Creator
    const { data: creator } = await supabase.from('creators').select('user_id').eq('id', creatorId).single();
    if (creator) {
      notify(creator.user_id, '💸 Payment Settled!', `Admin has manually settled your payment for "${jobTitle}". It is now in your Available Balance.`, 'payment');
    }

    console.log(`✅ Admin manually marked payment ${payment_id} as paid & credited wallet`);
    res.json({ success: true, message: 'Marked as paid and creator wallet credited!' });
  } catch (err) {
  ...
  try {
    ...
    if (creatorEmail) {
      sendEmail(creatorEmail, applicationStatusEmail({
        ...
        dashboardLink: `${getSiteUrl(req)}/creator-dashboard.html`
      }));
    }
...
app.post('/api/payments/:id/release', auth, async (req, res) => {
  ...
  try {
    ...
    await supabase.from('payments').update({ 
      ...
      released_at: new Date().toISOString(), 
      note: `Business marked as paid via ${method || 'UPI'} | UPI: ${upi_id || 'N/A'} | TXN: ${txn_id || 'N/A'}. Awaiting manual settlement by admin.` 
    }).eq('id', req.params.id);

    // Mark contract complete
    const { data: pRec } = await supabase.from('payments').select('contract_id').eq('id', req.params.id).single();
    if (pRec?.contract_id) {
      await supabase.from('contracts').update({
        status: 'completed',
        completed_at: new Date().toISOString(),
        business_done: true
      }).eq('id', pRec.contract_id);
    }
    
    const { data: payment } = await supabase.from('payments').select('*, contracts(id,agreed_amount,creator_id,jobs(title),creators(name,user_id),businesses(business_name))').eq('id', req.params.id).single();
    ...
    // ── Update creator wallet: move from pending → awaiting settlement ──
    try {
      if (payment.contracts?.creator_id) {
        await settlePending(payment.contracts.creator_id, Number(amount || 0), payment.contracts.id);
      }
    } catch (wErr) { console.error('Wallet settle error (non-fatal):', wErr.message); }
    
    // REMOVED: Instant creator wallet credit/earning update
    /*
    if (payment.contracts?.creator_id) {
    ...
    */
    ...
    notify(creatorUserId, '🎉 Work Approved!', `${bizName} approved your work for "${jobTitle}"! ₹${amount} is now awaiting manual settlement by admin. Check "Pending Settlement" in your dashboard.`, 'payment');
    sendEmail(creatorEmail, {
      subject: `✅ Work Approved — ₹${amount} Awaiting Settlement`,
      html: `<h2>✅ Work Approved!</h2><p>${bizName} has approved your work for "${jobTitle}".</p><p>₹${amount} is now awaiting manual settlement by the SocialMitr team.</p>`,
      text: `Work approved for "${jobTitle}". ₹${amount} is awaiting manual settlement.`
    });
    res.json({ success: true, message: 'Payment approved! Admin will settle the payout manually.' });
  } catch (err) { res.status(500).json({ success: false, message: err.message }); }
});
...
app.post('/api/admin/mark-paid', async (req, res) => {
  ...
  const { payment_id, contract_id, note } = req.body;
  try {
    const { data: payment } = await supabase.from('payments').select('*, contracts(agreed_amount,creator_id,creators(user_id))').eq('id', payment_id).single();
    ...
    const creatorId = payment.contracts?.creator_id;
    if (creatorId) {
      await confirmManualPayout(creatorId, payment.contracts.agreed_amount, contract_id, note);
    }

    // Notify creator
    const creatorUserId = payment.contracts?.creators?.user_id;
    if (creatorUserId) {
      notify(creatorUserId, '💰 Payment Settled!', `Admin has marked your payment for ₹${Math.round(payment.contracts.agreed_amount * 0.93)} as settled.`, 'payment');
    }
  ...
  try {
    ...
    sendEmail(bizEmail, {
      ...
      html: `<div style="font-family:'Segoe UI',sans-serif;background:#0f0f0f;color:#f0ece4;padding:32px;border-radius:12px;max-width:560px">
        ...
        <div style="margin-top:20px"><a href="${getSiteUrl(req)}/business-dashboard.html" style="background:#06d6a0;color:#0a0a0a;padding:12px 24px;border-radius:10px;text-decoration:none;font-weight:700;font-size:14px">Review Delivery →</a></div>
      </div>`,
...
app.get('/api/ai/test', auth, aiLimiter, async (req, res) => {
  try {
    const text = await callGroq('Say exactly: "SocialMitr AI is working!" and nothing else.', 50);
...
app.post('/api/razorpay/release-payment', auth, async (req, res) => {
  ...
  try {
    ...
    if (!payment) return res.status(404).json({ success: false, message: 'Payment not found' });
    if (!['held', 'escrowed', 'release_requested', 'requested'].includes(payment.status))
      return res.status(400).json({ success: false, message: `Cannot release — payment status is "${payment.status}"` });

    const { data: contract } = await supabase.from('contracts')
      .select('*, jobs(title), creators(id,name,user_id,upi_id,bank_account,bank_ifsc), businesses(business_name,user_id)')
      .eq('id', contract_id).single();
    ...
    // ── TRIGGER RAZORPAY PAYOUT (SocialMitr Payouts) ────────────────────────
    let payoutStatus = 'awaiting_manual_payout';
    let payoutId = '';
    let payoutNote = '';

    // Commenting out auto-payout logic for temporary manual system
    /*
    try {
      console.log(`🚀 Initiating Payout: ₹${creatorShare} to ${contract.creators.name}`);
      const payoutResult = await initiateRazorpayPayout({
        amount: creatorShare,
        upiId: contract.creators.upi_id,
        bankAccount: contract.creators.bank_account,
        bankIfsc: contract.creators.bank_ifsc,
        creatorName: contract.creators.name,
        referenceId: `rel_${contract_id.slice(0,8)}_${Date.now()}`
      });

      if (payoutResult.success) {
        payoutStatus = 'released';
        payoutId = payoutResult.payout_id;
        payoutNote = `Auto-split success: ₹${creatorShare} sent to creator (93%), ₹${platformShare} platform fee (7%). Razorpay Payout ID: ${payoutId}`;
        console.log(`✅ Payout Successful: ${payoutId}`);
      } else {
        payoutStatus = 'release_payout_failed';
        payoutNote = `Auto-split FAILED: ${payoutResult.error}. 93% share (₹${creatorShare}) needs manual payout. Platform fee: ₹${platformShare}`;
        console.error(`❌ Payout Failed: ${payoutResult.error}`);
      }
    } catch (pErr) {
      payoutStatus = 'release_payout_error';
      payoutNote = `Auto-split Error: ${pErr.message}. 93% share (₹${creatorShare}) needs manual check.`;
      console.error(`❌ Payout Error: ${pErr.message}`);
    }
    */
    payoutNote = `Awaiting manual settlement: ₹${creatorShare} (93%), Platform fee: ₹${platformShare} (7%).`;
    ...
    // ── Update creator wallet: move from pending → awaiting settlement ──
    try {
      const creatorId = contract.creators?.id;
      if (creatorId) {
        await settlePending(creatorId, contract.agreed_amount, contract_id);
        console.log(`✅ Wallet updated: ₹${creatorShare} moved from pending for creator ${creatorId}`);
      }
    } catch (wErr) { 
      console.error('Wallet settle error (non-fatal):', wErr.message); 
    }
    ...
    */
    ...
    // Notify creator
    const creatorUserId = contract.creators.user_id;
    notify(creatorUserId, '🎉 Work Approved!',
      `${contract.businesses.business_name} approved your work! ₹${creatorShare} is now awaiting manual settlement by admin. Check "Pending Settlement" in your dashboard.`,
      'payment'
    );
    ...
    if (creatorUser?.email) {
      sendEmail(creatorUser.email, {
        subject: `✅ Work Approved — ₹${creatorShare} Awaiting Settlement`,
        html: `<div style="font-family:'Segoe UI',sans-serif;background:#0f0f0f;color:#f0ece4;padding:32px;border-radius:12px;max-width:560px">
          <h2 style="color:#06d6a0;margin:0 0 16px">✅ Work Approved!</h2>
          ...
          <div style="background:#1e1e1e;border:1px solid #2a2a2a;border-radius:12px;padding:20px;margin:20px 0">
            ...
            <div style="display:flex;justify-content:space-between;font-size:1.1rem;font-weight:700;border-top:1px solid #333;padding-top:10px"><span style="color:#06d6a0">Pending Settlement</span><span style="color:#06d6a0">₹${creatorShare}</span></div>
          </div>
          <p style="color:#666;font-size:12px">Funds are currently awaiting manual settlement by the SocialMitr team. You will be notified once the transfer is completed.</p>
        </div>`,
        text: `Work approved for "${contract.jobs.title}". ₹${creatorShare} is awaiting manual settlement.`
      });
    }

    console.log(`\n✅ Payment approved: ₹${creatorShare} pending manual payout`);
    res.json({
      success: true,
      message: `Work approved! ₹${creatorShare} is now awaiting manual settlement.`,
      creatorShare,
      platformShare,
      totalAmount: amount
    });
