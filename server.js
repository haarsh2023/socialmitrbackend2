...
const { registerWalletRoutes, creditWallet, pendingCredit, confirmPendingCredit, initiateRazorpayPayout, createLinkedAccount } = require('./wallet');
...
app.post('/api/razorpay/release-payment', auth, async (req, res) => {
  ...
  try {
    ...
    const { data: contract } = await supabase.from('contracts')
      .select('*, jobs(title), creators(name,user_id,upi_id,bank_account,bank_ifsc,razorpay_account_id), businesses(business_name,user_id)')
      .eq('id', contract_id).single();
    ...
    try {
      console.log(`🚀 Initiating Route Transfer: ₹${creatorShare} to ${contract.creators.name}`);
      const payoutResult = await initiateRazorpayPayout({
        ...
        razorpayPaymentId:  payment.razorpay_payment_id,          // ← from payments table
        razorpayAccountId:  contract.creators.razorpay_account_id, // ← linked account
        onHold:             0  // release immediately on approval
      });
