 ...
 * Modified to directly credit to available_balance to allow immediate withdrawal.
 */
async function pendingCredit(creatorId, amount, referenceId, description) {
  await creditWallet(creatorId, amount, referenceId, description);
}
...
/**
 * initiateRazorpayPayout — calls Razorpay Payouts API to send money to creator.
 * Requires Razorpay Payouts to be activated on the account.
 * @param {object} opts
 */
async function initiateRazorpayPayout({ amount, upiId, bankAccount, bankIfsc, creatorName, referenceId }) {
  const keyId     = process.env.RAZORPAY_KEY_ID;
  const keySecret = process.env.RAZORPAY_KEY_SECRET;
  const accountId = process.env.RAZORPAY_ACCOUNT_NUMBER; // your Razorpay X account number

  if (!keyId || !keySecret || !accountId) {
    // Razorpay Payouts not configured — return mock success for now
    console.warn('⚠️ Razorpay Payouts not fully configured (need RAZORPAY_ACCOUNT_NUMBER). Marking as processing.');
  ...
  return new Promise((resolve) => {
    ...
    const body = JSON.stringify({
      ...
      reference_id:   referenceId,
      narration:      `SocialMitr payout - ${referenceId}`,
    });
    ...
    const options = {
      ...
      headers:  {
        ...
        'Content-Length':    Buffer.byteLength(body),
        'Authorization':     `Basic ${auth}`,
        'X-Payout-Idempotency': referenceId, // prevent duplicates
      },
    };
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          const parsed = JSON.parse(data);
          if (parsed.id) {
            resolve({ success: true, payout_id: parsed.id, status: parsed.status });
          } else {
            resolve({ success: false, error: parsed.error?.description || 'Payout failed' });
          }
        } catch {
          resolve({ success: false, error: 'Invalid response from Razorpay' });
        }
      });
    });
  ...
  app.put('/api/creators/payout-info', auth, async (req, res) => {
    ...
    try {
      ...
      if (payout_method === 'bank' || (bank_account && bank_ifsc)) {
        ...
        if (!updateData.payout_method) updateData.payout_method = 'bank';
      }
