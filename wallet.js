...
async function pendingCredit(creatorId, amount, referenceId, description) {
  ...
  const { error: tErr } = await supabase
    ...
    .insert({
      ...
      reference_id:   referenceId,
      reference_type: 'contract',
      payout_method:  (await supabase.from('creators').select('payout_method').eq('id', creatorId).single()).data?.payout_method || 'upi'
    });
  if (tErr) throw tErr;
...
// ═══════════════════════════════════════════════════════════════════════════════
/**
 * confirmManualPayout — admin marks a pending settlement as paid.
 * Moves from awaiting_settlement → confirmed, and updates available balance.
 */
async function confirmManualPayout(creatorId, amount, referenceId, note) {
  const wallet = await getOrCreateWallet(creatorId);
  const creatorShare = Math.round(amount * 0.93);

  // Update wallet available balance and total earned
  const newAvailable = Number(wallet.available_balance) + creatorShare;
  const newTotalEarned = Number(wallet.total_earned) + creatorShare;
  ...
  await supabase.from('wallet_transactions')
    .update({ 
      status: 'completed', 
      description: note || `Payout settled manually: ₹${creatorShare}`,
      balance_after: newAvailable
    })
    .eq('reference_id', referenceId)
    .in('status', ['awaiting_settlement', 'pending']);

  return { newAvailable, creatorShare };
}
